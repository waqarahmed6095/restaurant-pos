...
class OrderEntryPanel:
    ...
    def _build(
        ...
        for row, label in ((0, "CATEGORY"), (2, "MENU ITEM"), (4, "SIZE")):
            ctk.CTkLabel(
                form_frame, text=label, text_color=colors["muted"], font=("Segoe UI", 9, "bold")
            ).grid(row=row, column=0, sticky="w", padx=12, pady=(12 if row == 0 else 4, 4))

        self.category_menu = ttk.Combobox(
            form_frame, textvariable=category_var, values=categories,
            state="readonly", style="Modern.TCombobox"
        )
        self.category_menu.grid(row=1, column=0, padx=12, pady=(0, 10), sticky="ew")
        self.item_menu = ttk.Combobox(
            form_frame, textvariable=item_var, state="readonly", style="Modern.TCombobox"
        )
        self.item_menu.grid(row=3, column=0, padx=12, pady=(0, 10), sticky="ew")
        self.size_menu = ttk.Combobox(
            form_frame, textvariable=size_var, state="readonly", style="Modern.TCombobox"
        )
        ...
        self.price_label = ctk.CTkLabel(
            form_frame, text="Unit price: Rs 0.00", text_color=colors["brand"], font=("Segoe UI", 13, "bold")
        )
        ...
        ctk.CTkButton(quantity_stepper, text="-", command=lambda: on_quantity_change(-1), width=38, height=38, corner_radius=7, fg_color=colors["brand"], hover_color=colors["brand_dark"], font=("Segoe UI", 16, "bold")).pack(side="left")
        ctk.CTkEntry(quantity_stepper, textvariable=quantity_var, width=44, height=38, justify="center", border_width=0, fg_color=colors["soft"], text_color=colors["ink"], font=("Segoe UI", 12, "bold")).pack(side="left", padx=2)
        ctk.CTkButton(quantity_stepper, text="+", command=lambda: on_quantity_change(1), width=38, height=38, corner_radius=7, fg_color=colors["brand"], hover_color=colors["brand_dark"], font=("Segoe UI", 16, "bold")).pack(side="left")
        ctk.CTkButton(self.frame, text="+  ADD TO ORDER", command=on_add, height=46, corner_radius=9, fg_color=colors["brand"], hover_color=colors["brand_dark"], font=("Segoe UI", 12, "bold")).grid(row=4, column=0, sticky="ew", padx=18, pady=(4, 18))

        table_frame = ctk.CTkFrame(self.frame, fg_color=colors["panel"], corner_radius=14)
        ...
        self.table_type_menu = ttk.Combobox(table_frame, textvariable=table_type_var, values=["Indoor", "Outdoor", "Home Delivery"], state="readonly", style="Modern.TCombobox")
        ...
        self.table_number_menu = ttk.Combobox(table_frame, textvariable=table_number_var, state="readonly", style="Modern.TCombobox")
        self.table_number_menu.grid(row=2, column=1, padx=(0, 16), pady=3, sticky="ew")
        self.service_charge_check = ctk.CTkCheckBox(table_frame, text="Add Service Charge", variable=on_service_charge_changed[0], command=on_service_charge_changed[1], text_color=colors["muted"], fg_color=colors["brand"], hover_color=colors["brand_dark"])
